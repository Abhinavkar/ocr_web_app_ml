...
class AnswerRetrieveAPI(APIView):
    def get(self, request):
        try:
            # Get parameters from the request
            roll_no = request.query_params.get('rollNo')
            ...
            # Query the MongoDB collection for the data
            answers_collection = get_collection("answers_db")
            
            # Find the answer data matching the provided filters
            query = {"roll_no": roll_no, "exam_id": exam_id}
            ...
            answer_data = answers_collection.find_one(query)
            
            if not answer_data:
                return Response({"message": "Answer data not found for the provided parameters."}, status=status.HTTP_404_NOT_FOUND)

            # Preparing the response data
            response_data = {
                "roll_no": answer_data.get("roll_no"),
                "exam_id": answer_data.get("exam_id"),
                "class_id": answer_data.get("class_id"),
                "subject": answer_data.get("subject"),
                "pdf_file_path": answer_data.get("pdf_file_path"),
                "extracted_text": answer_data.get("extracted_text"),
                "sentences": answer_data.get("sentences"),
                "user_answers": answer_data.get("user_answers"),
                "results": answer_data.get("results"),
                "user_answer_embeddings": answer_data.get("user_answer_embeddings"),
            }
